...
def destDir = new File( project.basedir, "target/generated-test-sources/groovy/onl/area51/geotools" );
mkdir(destDir);
...
sourceDir.eachDirRecurse {
    it.eachFileMatch( ~/.*\.job/ ) {
        ...
        if((m = n =~ /geotools\/([a-zA-Z][a-zA-Z_]*?)([0-9]*?).job$/))
            tests << m.group(1);
    }
}
...
tests.each {
    test ->
        ...
        new File( "$destDir/"+className+".java" ).withWriter { out ->
            out.println "package onl.area51.geotools;";
            out.println "";
